...
public class DetailActivity extends AppCompatActivity{
    ...
    public static SpellViewModel spellViewModel;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        character = (Character) Objects.requireNonNull(getIntent().getExtras()).getSerializable(CHARACTER);
        ...
        completeSpellsList.observe(this, new Observer<List<Spell>>() {
            @Override
            public void onChanged(List<Spell> spells) {
                                ...
                                if(!response.isSuccessful()){
                                    ...
                                    if(response.body() != null && !response.body().getResults().isEmpty()){
                                        for(int j=0; j<response.body().getResults().size(); j++){
                                            spellViewModel.insertSpell(response.body().getResults().get(j));
                ...
                Collections.sort(spells, Spell.spellNameComparator);
            }
        });
    }
    ...
    private void onLevelUpButtonClicked(){
        if(viewPager.getCurrentItem() == 0){
            fragmentTransaction.remove(fragmentManager.getFragments().get(0))
                    .add(R.id.tabsLayout, new LevelUpFragment(), "levelUpFragment")
                    .commit();
        } else if(viewPager.getCurrentItem() == 1 || viewPager.getCurrentItem() == 2 || viewPager.getCurrentItem() == 3){
